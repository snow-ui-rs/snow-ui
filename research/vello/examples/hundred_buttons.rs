...
fn build_scene(width: u32, height: u32, label: &ImageData) -> Scene {
    ...
    let cols = 10;
    let rows = 10;
    let cell_w = (width as f64 - 60.0) / cols as f64;
    let cell_h = (height as f64 - 60.0) / rows as f64;
    let button_w = cell_w - 16.0;
    let button_h = cell_h - 16.0;

    for row in 0..rows {
        for col in 0..cols {
            let x = 30.0 + col as f64 * cell_w;
            let y = 30.0 + row as f64 * cell_h;
            let button_rect = RoundedRect::new(x, y, x + button_w, y + button_h, 12.0);
            scene.fill(
                Fill::NonZero,
                Affine::IDENTITY,
            ...
            let image_x = x + (button_w - label.width as f64) / 2.0;
            let image_y = y + (button_h - label.height as f64) / 2.0;
...
fn main() {
    ...
    let window = WindowBuilder::new()
        .with_title("vello 100 buttons")
        .with_inner_size(PhysicalSize::new(1100, 900))
        .build(&event_loop)
    ...
    let mut scene = build_scene(1100, 900, &label_image);
                    ...
                    WindowEvent::RedrawRequested => {
                        ...
                        let mut encoder =
                            device.create_command_encoder(&wgpu::CommandEncoderDescriptor {
                                label: Some("Vello Hundred Buttons Encoder"),
                            });
